  ...
  "KafkaClient" - {
    "getGroupOffsets" - {
      "returns None offsets for missing partitions and doesn't overwrite results for shared topic partitions" in {
        ...
        val groupId0Results = Future.successful(
          Map(
            topicPartition0.asKafka -> new OffsetAndMetadata(
              0,
              Optional.empty(),
              ""
            )
        ...
        when(adminKafkaClient.listConsumerGroupOffsets(groupId0))
          .thenReturn(groupId0Results)

        val groupId1Results = Future.successful(
          Map(
            topicPartition0.asKafka -> new OffsetAndMetadata(
              1,
              Optional.empty(),
        ...
        when(adminKafkaClient.listConsumerGroupOffsets(groupId1))
          .thenReturn(groupId1Results)
      ...
      "returns no offsets when a null offset is returned" in {
        ...
        val listGroupOffsetResults = Future.successful(
          Map[KafkaTopicPartition, OffsetAndMetadata](
            topicPartition0.asKafka -> null
          ).asJava
        )
        when(adminKafkaClient.listConsumerGroupOffsets(groupId))
          .thenReturn(listGroupOffsetResults)
    ...
    "getOffsetOrZero returns offsets of None (Option[Point]) for missing partitions" in {
      ...
      val groupOffsets =
        client.getOffsetOrZero(List(gtp0, gtp1, gtp2), offsetMap)
    ...
    "groupTopicPartition" - {
      ...
      "will default to fetching all topics" in {
        ...
        val tps = Set(
          ...
          topicPartition2,
          topic2Partition0
        ).map(tp => new KafkaTopicPartition(tp.topic, tp.partition)).asJava
        val members = List(
        ...
        val description = new ConsumerGroupDescription(
          groupId,
          true,
          members,
          "",
        ...
        client.groupTopicPartitions(
          groupId,
          description
        ) should contain theSameElementsAs List(gtp0, gtp1, gtp2, gt2p0).map(
          gtp => gtp.copy(host = tmpHost)
        )
      }
      ...
      "will only fetch whitelisted topic when whitelist contains a single topic" in {
        ...
        client.groupTopicPartitions(
          groupId,
          description
        ) should contain theSameElementsAs List(gt2p0).map(gtp =>
          gtp.copy(host = tmpHost)
        )
      }

      "will only fetch non blacklisted topics" in {
        val tmpCluster: KafkaCluster = cluster.copy(
          topicWhitelist = List(topic, topic2),
          topicBlacklist = List(topic)
        )
        val (_, _, client) = getClient(tmpCluster)
        val tps = Set(
          topicPartition0,
      ...
      "will exclude topics that match with blacklist regex" in {
        val tmpCluster = cluster.copy(
          topicWhitelist = List("test.+"),
          topicBlacklist = List(".+topic-2")
        )
        val (_, _, client) = getClient(tmpCluster)
        val tps = Set(
          topicPartition0,
        ...
        client.groupTopicPartitions(
          groupId,
          description
        ) should contain theSameElementsAs List(gtp0, gtp1, gtp2).map(gtp =>
          gtp.copy(host = tmpHost)
        )
      }
    ...
    "getGroupIds" - {
      ...
      "will default to fetching all group IDs" in {
        ...
        groupIds should contain theSameElementsAs List(
          groupId1,
          groupId2,
          groupIdIotData
        )
      ...
      "will only fetch whitelisted and non blacklisted groups" in {
        val tmpCluster = cluster.copy(
          groupWhitelist = List(groupId1, groupId2),
          groupBlacklist = List(groupId2, groupIdIotData)
        )
        val (_, _, client) = getClient(tmpCluster)
        val groupIds = client.getGroupIds(groups)
        groupIds should contain theSameElementsAs List(groupId1)
      ...
      "will include groups that match with whitelist regex and exclude groups that match with blacklist regex" in {
        val tmpCluster = cluster.copy(
          groupWhitelist = List("iot-.+"),
          groupBlacklist = List("testGroupId[0-9]")
        )
        val (_, _, client) = getClient(tmpCluster)
        val groupIds = client.getGroupIds(groups)
        groupIds should contain theSameElementsAs List(groupIdIotData)
